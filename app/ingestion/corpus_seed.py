...
**Lives in `app/`, not `evals/`, since M2.** Originally
`evals/retrieval/corpus.py`; `app/demo/seed.py` needed this exact,
unduplicated seeding logic to seed the demo corpus, and
`tests/test_retrieval_scope.py::test_no_api_module_imports_the_evals_package`
enforces the layering rule this project has always had — the application
never imports `evals/`, only the reverse. Moving the implementation here
and making `evals/retrieval/corpus.py` a thin re-export keeps every
existing `evals`/test caller working unchanged while giving `app/` a
legitimate way to reuse it. `DEFAULT_MANIFEST` still resolves to
`evals/fixtures/knowledge_base/manifest.yaml` — a file path, not a Python
import — the fixture corpus itself is still owned by `evals/`.
"""
...
from app.models import Chunk, DocumentVersion, VersionStatus
from app.providers.embeddings import EmbeddingProvider
from app.storage import Storage

# `parents[2]` from `app/ingestion/corpus_seed.py` is the repository root;
# the fixture corpus itself still lives under `evals/`, which this module
# only ever reads a file path from, never imports.
DEFAULT_MANIFEST = (
